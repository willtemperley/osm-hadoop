...
package org.openstreetmap.osmosis.pbf;
 ...
 */
public class PbfBlobDecoder2 {

	private static Logger log = Logger.getLogger(PbfBlobDecoder2.class.getName());
	 ...
	 */
	public PbfBlobDecoder2(String blobType, byte[] rawBlob) {
	...
	private CommonEntityData buildCommonEntityData(long entityId, List<Integer> keys, List<Integer> values, Osmformat.Info info,
			PbfFieldDecoder fieldDecoder) {
		OsmUser user;
	...
	private void processNodes(List<Osmformat.Node> nodes, PbfFieldDecoder fieldDecoder) {
		for (Osmformat.Node node : nodes) {
			...
			osmNode = new Node(entityData, fieldDecoder.decodeLatitude(node
	...
	private void buildRelationMembers(Relation relation,
	...
	private void processRelations(List<Osmformat.Relation> relations, PbfFieldDecoder fieldDecoder) {
		for (Osmformat.Relation relation : relations) {
			...
			osmRelation = new Relation(entityData);
	...
	public void runAndTrapExceptions(ArrayList<EntityContainer> decodedEntities) {
		try {
			this.decodedEntities = decodedEntities;
