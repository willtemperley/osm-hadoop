...
import org.roadlessforest.osm.grid.{CoordinateWritable, GlobalGrid}

/**
  * Created by willtemperley@gmail.com on 04-Mar-16.
  */
object ExtractRasterAfr {
  ...
  def main(args: Array[String]) {
    ...
    val pixelsD: RDD[(CoordinateWritable, IntWritable)] = sc.sequenceFile(sequenceFileInputPath, classOf[CoordinateWritable], classOf[IntWritable])

    val pix: Array[(CoordinateWritable, Int)] = pixelsD.map(f => (f._1, f._2.get())).collect()

    val pixelsC: Array[(Int, Int, Int)] = pixelsD
